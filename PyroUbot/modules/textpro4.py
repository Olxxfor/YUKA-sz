...
async def process_image_command(client, message, api_url, command_name):
    ...
    if len(args) < 2:
        await message.reply(f"<blockquote><b><i>Gunakan perintah /{command_name} <teks> untuk membuat gambar.</i></b></blockquote>")
        return

    request_text = args[1]
    await message.reply("<blockquote><b><i>Sedang memproses, mohon tunggu...</i></b></blockquote>")
    ...
    if image_content:
        ...
        await message.reply("<blockquote>Gagal membuat gambar. Coba lagi nanti.</blockquote>")
